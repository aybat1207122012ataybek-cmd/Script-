...
local function ManagePlatform()
    ...
    if vDir ~= 0 or (plat and plat.Parent) then
        if not plat or not plat.Parent then
            ...
            plat.Size = Vector3.new(12, 1, 12) plat.Anchored = true 
        end
        plat.CanCollide = true plat.Transparency = 0.5 plat.Color = _G.PlatColor
        
        local targetY = (vDir ~= 0) and (plat.Position.Y + (vDir * 1.0)) or plat.Position.Y
        plat.CFrame = plat.CFrame:Lerp(CFrame.new(root.Position.X, targetY, root.Position.Z), 0.5)
    end
end

local function CreateUI()
    local pg = lp:WaitForChild("PlayerGui", 20)
    if pg:FindFirstChild("Dandy_V51_Lite") then return end
    local sg = Instance.new("ScreenGui", pg) sg.Name = "Dandy_V51_Lite" sg.ResetOnSpawn = false
    ...
    local hueLine = Instance.new("Frame", hueS) hueLine.Size = UDim2.new(0, 2, 1, 4) hueLine.BackgroundColor3 = Color3.new(1,1,1) hueLine.ZIndex = 6
    ...
    local function update()
        ...
        hueLine.Position = UDim2.new(cH, 0, 0.5, 0)
    ...
    local function setupPicker(obj, func)
        ...
        RS.RenderStepped:Connect(function() if active and colorPanel.Visible then proc() end end)
    end

    setupPicker(hueS, function(x) cH = x update() end)
    setupPicker(picker, function(x, y) cS = x cV = 1 - y update() end)
    setupPicker(valS, function(x, y) cV = 1 - y update() end)
    ...
    lock.MouseButton1Click:Connect(function() _G.GuiLocked = not _G.GuiLocked lock.TextLabel.Text = _G.GuiLocked and "UNFIX" or "FIXED" lock.BackgroundColor3 = _G.GuiLocked and Color3.fromRGB(150, 0, 0) or Color3.fromRGB(30, 30, 30) end)
